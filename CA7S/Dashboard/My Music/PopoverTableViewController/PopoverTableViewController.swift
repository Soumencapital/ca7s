...
public class PopoverTableViewController: UITableViewController {
    ...
    public var contentInset: UIEdgeInsets = UIEdgeInsets(top: 0, left: 10, bottom: 0, right: 10)
    
    override public var preferredContentSize: CGSize {
        get {
            
            //UPDATED BY MANSI
            var width = (items.map { $0.width }.max() ?? 0) + contentInset.left + contentInset.right
            var height: CGFloat = (CGFloat(items.count) * rowHeight - 1) + contentInset.top + contentInset.bottom
            if width > 285{
                width = 285
                height = height + (10)
            }
